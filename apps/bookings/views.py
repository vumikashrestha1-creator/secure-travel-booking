...
class AdminCreateBookingView(APIView):
    ...
    def post(self, request):
        ...
        if listing.status != "ACTIVE":
            return Response({"error": "This listing is not available."}, status=400)

        if number_of_guests > listing.available_seats:
            return Response({"error": "Not enough seats available."}, status=400)

        serializer = CreateBookingSerializer(
            data={"listing": listing_id, "number_of_guests": number_of_guests},
        ...
        if serializer.is_valid():
            ...
            listing.save()
            return Response({
                "message":   "Booking created successfully for " + customer.email,
                "reference": booking.booking_reference,
            }, status=201)
