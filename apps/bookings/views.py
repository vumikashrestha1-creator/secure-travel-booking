...
class CreateBookingView(APIView):
    ...
    def post(self, request):
        ...
        if listing.status != "ACTIVE":
            return Response({"error": "This listing is no longer available."}, status=status.HTTP_400_BAD_REQUEST)

        if listing.available_seats <= 0:
            return Response({"error": "Sorry this package is fully booked."}, status=status.HTTP_400_BAD_REQUEST)

        if number_of_guests > listing.available_seats:
            return Response(
                {"error": f"Not enough seats. Only {listing.available_seats} seats remaining."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if number_of_guests < 1:
            return Response({"error": "Number of guests must be at least 1."}, status=status.HTTP_400_BAD_REQUEST)
...
class MyBookingsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        queryset = Booking.objects.filter(user=self.request.user).select_related("listing", "user")
...
class RescheduleBookingView(APIView):
    ...
    def patch(self, request, pk):
        ...
        if booking.status == Booking.Status.CANCELLED:
            return Response({"error": "Cancelled booking cannot be rescheduled."}, status=status.HTTP_400_BAD_REQUEST)

        if booking.status == Booking.Status.COMPLETED:
            return Response({"error": "Completed booking cannot be rescheduled."}, status=status.HTTP_400_BAD_REQUEST)

        new_date = request.data.get("travel_date")

        if not new_date:
            return Response({"error": "Travel date is required."}, status=status.HTTP_400_BAD_REQUEST)

        booking.listing.start_date = new_date
